...
def reduce_sum2d_conv(inputs,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as sc:
        ...
        outputs = tf.reduce_sum(inputs, axis=axis, keep_dims=keepdims, name=sc.name)
  ...
  """
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as sc:
        ...
        beta = tf.get_variable(name='beta',
                               initializer=tf.constant(0.0, shape=[num_channels]),
                               trainable=True)
        gamma = tf.get_variable(name='gamma',
                                initializer=tf.constant(1.0, shape=[num_channels]),
                                trainable=True)
